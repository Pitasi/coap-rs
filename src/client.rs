...
macro_rules! method {
  // Wrapper for calling the macro stringifying the identifiers.
  ($name:ident, $name_with_timeout:ident, $method:ident) => {
    method!(
      $name,
      $name_with_timeout,
      $method,
  ...
  ($name:ident, $name_with_timeout:ident, $method:ident, with_payload) => {
    method!(
      $name,
      $name_with_timeout,
      $method,
  ...
  // Method without payload variant
  ($name:ident, $name_with_timeout:ident, $method:ident, $sname:expr) => {
    #[doc="Execute a `"]
    #[doc=$sname]
    #[doc="` request to the specified url."]
    #[doc="The default timeout will be used for waiting a response."]
    pub fn $name(url: &str) -> Result<CoAPResponse> {
    ...
    #[doc=$sname]
    #[doc="` request to the specified url."]
    #[doc="Specify a `timeout` to be used for waiting a response."]
    pub fn $name_with_timeout(url: &str, timeout: Duration)
      -> Result<CoAPResponse> {
      Self::request_with_timeout(
        Method::$method,
        url,
  ...
  // Method with payload variant
  (
     $name:ident,
     $name_with_timeout:ident,
     $method:ident,
     $sname:expr,
     with_payload
  ) => {
    ...
    #[doc=$sname]
    #[doc="` request to the specified url."]
    #[doc="A `payload` will be sent within the message."]
    #[doc="The default timeout will be used for waiting a response."]
    pub fn $name(url: &str, payload: Vec<u8>) -> Result<CoAPResponse> {
      Self::request_with_timeout(
        Method::$method,
        url,
        Some(payload),
    ...
    #[doc="A `payload` will be sent within the message."]
    #[doc="Specify a `timeout` to be used for waiting a response."]
    pub fn $name_with_timeout(url: &str, payload: Vec<u8>, timeout: Duration)
      -> Result<CoAPResponse> {
      Self::request_with_timeout(
        Method::$method,
        url,
...
impl CoAPClient {
  ...
  method!(get, get_with_timeout, Get);
  method!(delete, delete_with_timeout, Delete);
  method!(post, post_with_timeout, Post, with_payload);
  method!(put, put_with_timeout, Put, with_payload);

  /// Execute a request to the specified url using the specified method,
  /// sending a payload if any, and waiting the response for a certain timeout.
  pub fn request_with_timeout(
    ...
    let token_size = 4;
    let token: Vec<u8> = (0..token_size).into_iter().map(|_| random()).collect();
    ...
    let check_id = |packet: &CoAPResponse|
      packet.get_message_id() == message_id;
    let check_token = |packet: &CoAPResponse|
      packet.get_token() == &token;

    match client.receive() {
      Ok(received) => {
        if check_id(&received) && check_token(&received) {
          Ok(received)
        } else {
          Err(Error::new(ErrorKind::Other, "received invalid data"))
        }
      }
      err => err
    }
